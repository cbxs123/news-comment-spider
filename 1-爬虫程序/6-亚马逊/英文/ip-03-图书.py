...
def log(ma):
    file = open(r'03-图书.txt', mode='a+', encoding='utf-8')
    file.write(ma+'\n')
    file.close()
...
def foo(skuj):
    try:
        url3 = "https://www.amazon.cn/dp/" + str(skuj) + '/ie=UTF8'
            ...
            try:
                ...
                try:
                    with connection.cursor() as cursor:
                        sql = " insert into `amazon_cn_03tushu`(`proid`,`prourl`,`goodRateShow`,`commentCount`," \
...
if __name__ == "__main__":
    for k in range(21,400):
        ips = [line.strip() for line in open(r"ips-1.txt", "r")]
        if len(ips)<10:
            ips = [line.strip() for line in open(r"ips.txt", "r")]
        time.sleep(random.random() * 10)
        url = "https://www.amazon.cn/s/ref=sr_pg_" + str(k) + "?rh=n%3A658390051%2Ck%3A%E5%9B%BE%E4%B9%A6&page=" + str(k) + "&sort=review-rank&keywords=%E5%9B%BE%E4%B9%A6&ie=UTF8&lo=stripbooks"
        #      https://www.amazon.cn/s/ref=sr_pg_3?rh=n%3A658390051%2Ck%3A%E5%9B%BE%E4%B9%A6&page=3&sort=review-rank&keywords=%E5%9B%BE%E4%B9%A6&ie=UTF8&lo=stripbooks
        try:
            root=get_url1(url,ips)
            sku = root.xpath('//div[@id="atfResults"]//@data-asin')
            n_thread = random.randint(100,250)
