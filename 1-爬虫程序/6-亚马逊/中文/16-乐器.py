...
def log(ma):
    file = open('16_乐器.txt', mode='a+', encoding='utf-8')
    file.write(ma+'\n')
    file.close()
...
def foo(host):
    ...
    try:
        res = requests.get(test_url, headers=headers(), proxies=proxy, timeout=20)
        code = res.status_code
        if code == 200:
            print("OK---" + host)
            file = open('ips-16.txt', mode='a+', encoding='utf-8')
            file.write(host + '\n')
            file.close()
    except:
        print("NO***" + host)
...
def ips():
    try:
        ...
        nameList = [line.strip() for line in open(r"thebigproxylist-18-10-06.txt", "r")]
    n_thread = 200
            ...
            try:
                ...
                try:
                    with connection.cursor() as cursor:
                        sql = " insert into `amazon_cn_16yueqi`(`proid`,`prourl`,`goodRateShow`,`commentCount`," \
...
if __name__ == "__main__":
    for k in range(1,400):
        ...
        ips = [line.strip() for line in open(r"ips-16.txt", "r")]
        try:
            ma='1---正在获取第' + str(k) + '页'+time.asctime(time.localtime(time.time()))
            print(ma)
            log(ma)
            url="https://www.amazon.cn/s/ref=sr_pg_"+str(k)+"?rh=n%3A2127218051%2Ck%3A%E4%B9%90%E5%99%A8&page="+str(k)+"&sort=review-rank&ie=UTF8&lo=mi"
