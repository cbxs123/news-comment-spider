...
def log(ma):
    file = open(r'02-鞋靴.txt', mode='a+', encoding='utf-8')
    file.write(ma+'\n')
    file.close()
            ...
            try:
                ...
                try:
                    with connection.cursor() as cursor:
                        sql = " insert into `amazon_cn_02xiexue`(`proid`,`prourl`,`goodRateShow`,`commentCount`," \
...
if __name__ == "__main__":
    for k in range(1,400):
        ...
        print(ma)
        log(ma)
        url = "https://www.amazon.cn/s/ref=sr_pg_" + str(k) + "?rh=n%3A2029189051%2Ck%3A%E9%9E%8B%E9%9D%B4&page=" + str(k) + "&sort=review-rank&keywords=%E9%9E%8B%E9%9D%B4&ie=UTF8"
        #    https://www.amazon.cn/s/ref=sr_pg_4?sf=sbc%2Cqz&rh=n%3A1953164051%2Ck%3A%E9%92%9F%E8%A1%A8&page=4&sort=review-rank&keywords=%E9%92%9F%E8%A1%A8&ie=UTF8
        try:
            root=get_url1(url,ips)
            sku = root.xpath('//div[@id="atfResults"]//@data-asin')
            n_thread = random.randint(300,500)
            thread(n_thread, sku)
        except:
