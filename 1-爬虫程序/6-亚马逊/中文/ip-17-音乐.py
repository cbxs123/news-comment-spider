...
def log(ma):
    file = open(r'17_音乐.txt', mode='a+', encoding='utf-8')
    file.write(ma+'\n')
    file.close()
...
def foo(host):
    ...
    try:
        ...
        if code == 200:
            print("OK---" + host)
            file = open('ips-17.txt', mode='a+', encoding='utf-8')
            file.write(host + '\n')
            file.close()
    except:
        print("NO***" + host)
            ...
            try:
                ...
                try:
                    with connection.cursor() as cursor:
                        sql = " insert into `amazon_cn_17yinyue`(`proid`,`prourl`,`goodRateShow`,`commentCount`," \
...
if __name__ == "__main__":
    for k in range(60,400):
        ...
        try:
            file = open(r'ips-17.txt', mode='w+', encoding='utf-8')
            file.close()
            foo=foo
            ips()
            ips = [line.strip() for line in open(r"ips-17.txt", "r")]
            if len(ips)<10:
                ips = [line.strip() for line in open(r"ips.txt", "r")]
        except:
            ips = [line.strip() for line in open(r"ips.txt", "r")]
        ...
        try:
            ...
            print(ma)
            log(ma)
            url="https://www.amazon.cn/s/ref=sr_pg_"+str(k)+"?rh=n%3A754386051&page="+str(k)+"&sort=review-rank&ie=UTF8&lo=music"
