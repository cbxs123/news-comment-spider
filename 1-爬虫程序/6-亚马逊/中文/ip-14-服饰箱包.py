...
def log(ma):
    file = open(r'14_服饰箱包.txt', mode='a+', encoding='utf-8')
    file.write(ma+'\n')
    file.close()
...
def foo(host):
    ...
    try:
        ...
        if code == 200:
            print("OK---" + host)
            file = open('ips-14.txt', mode='a+', encoding='utf-8')
            file.write(host + '\n')
            file.close()
    except:
        print("NO***" + host)
            ...
            try:
                ...
                try:
                    with connection.cursor() as cursor:
                        sql = " insert into `amazon_cn_14fushi`(`proid`,`prourl`,`goodRateShow`,`commentCount`," \
...
if __name__ == "__main__":
    for k in range(1,400):
        ...
        try:
            file = open(r'ips-14.txt', mode='w+', encoding='utf-8')
            file.close()
            foo=foo
            ips()
            ips = [line.strip() for line in open(r"ips-14.txt", "r")]
            if len(ips)<10:
                ips = [line.strip() for line in open(r"ips.txt", "r")]
        except:
            ips = [line.strip() for line in open(r"ips.txt", "r")]
        ...
        try:
            ...
            print(ma)
            log(ma)
            url="https://www.amazon.cn/s/ref=sr_pg_"+str(k)+"?rh=n%3A2016156051%2Ck%3A%E6%9C%8D%E9%A5%B0%E7%AE%B1%E5%8C%85&page="+str(k)+"&sort=review-rank&keywords=%E6%9C%8D%E9%A5%B0%E7%AE%B1%E5%8C%85&ie=UTF8"
