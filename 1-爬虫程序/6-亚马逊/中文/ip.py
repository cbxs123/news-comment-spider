...
def foo(host):
    ...
    try:
        ...
        if code == 200:
            print("OK---" + host)
            file = open('ips-18.txt', mode='a+', encoding='utf-8')
            file.write(host + '\n')
            file.close()
    except:
        print("NO***" + host)
...
if __name__ == "__main__":
    for k in range(117,400):
        time.sleep(random.random()*10)
        try:
            file = open(r'ips-18.txt', mode='w+', encoding='utf-8')
            file.close()
            foo=foo
            ips()
            ips = [line.strip() for line in open(r"ips-18.txt", "r")]
            if len(ips)<10:
                ips = [line.strip() for line in open(r"ips.txt", "r")]
        except:
            ips = [line.strip() for line in open(r"ips.txt", "r")]
