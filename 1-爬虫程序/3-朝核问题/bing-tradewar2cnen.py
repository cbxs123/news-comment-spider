...
def log(ma):
    file = open(r'必应-朝核问题.txt', mode='a+', encoding='utf-8')
    file.write(ma+'\n')
    file.close()
...
def foo(u):
    ...
    j2=int(int(s2-int(time.mktime(time.strptime("2003-03-01 00:00:00", "%Y-%m-%d %H:%M:%S"))))/86400)
    j1= j2+12112
    r=list(range(1,30))
    random.shuffle(r)
    ...
    for m in r:
        try:
            url = 'https://www.bing.com/search?q=North+Korea+nuclear+&filters=ex1%3a%22ez5_' + str(j1) + '_' + str(
                j1 + q) + '%22&pq=North+Korea+nuclear+&qpvt=North+Korea+nuclear+&ensearch=0&first=' + str(m*10)
            driver.get(url)
            time.sleep(random.random() * 5)
            ...
            pass
        try:
            url = 'https://cn.bing.com/search?q=North+Korea+nuclear+&filters=ex1%3a%22ez5_' + str(j1) + '_' + str(
                j1 + q) + '%22&pq=North+Korea+nuclear+&qpvt=North+Korea+nuclear+&ensearch=1&first=' + str(m*14)
            driver.get(url)
            time.sleep(random.random() * 5)
            html = driver.page_source
...
if __name__ == "__main__":
    ...
    if g==1:
        w,p,q,n1,n2,s,e=0,2,30,10,14,'2003-03-01','2018-10-31'
    else:
