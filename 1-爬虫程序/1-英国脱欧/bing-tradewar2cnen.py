...
os.chdir("D:/1/毕业论文/180430-KT/数据/05-英国脱欧")

def log(ma):
    file = open(r'必应-英国脱欧.txt', mode='a+', encoding='utf-8')
    file.write(ma+'\n')
    file.close()
...
def foo(u):
    ...
    def foo1(i):
        ...
        try:
            ...
            connection = pymysql.connect(host='localhost', user='root', password='asd123', db='king',charset='utf8mb4')
            try:
    ...
    j2=int(int(s2-int(time.mktime(time.strptime("2013-01-23 00:00:00", "%Y-%m-%d %H:%M:%S"))))/86400)
    j1= j2+15728
    r=list(range(1,30))
    random.shuffle(r)
    ...
    for m in r:
        try:
            url = 'https://www.bing.com/search?q=Britain+exiting+from+the+EU+Brexit&filters=ex1%3a%22ez5_' + str(j1) + '_' + str(
                j1 + q) + '%22&pq=britain+exiting+from+the+eu+brexit&qpvt=Britain+exiting+from+the+EU+Brexit&ensearch=0&first=' + str(m*10)
            driver.get(url)
            time.sleep(random.random() * 5)
            ...
            pass
        try:
            url = 'https://cn.bing.com/search?q=Britain+exiting+from+the+EU+Brexit&filters=ex1%3a%22ez5_' + str(j1) + '_' + str(
                j1 + q) + '%22&pq=britain+exiting+from+the+eu+brexit&qpvt=Britain+exiting+from+the+EU+Brexit&ensearch=1&first=' + str(m*14)
            driver.get(url)
            time.sleep(random.random() * 5)
            html = driver.page_source
...
if __name__ == "__main__":
    ...
    if g==1:
        w,p,q,n1,n2,s,e=0,2,27,10,14,'2013-01-23','2016-06-30'
    else:
