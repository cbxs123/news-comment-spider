...
for k in range(0,60):
    ...
    url="https://www.joybuy.com/search?keywords=book&sortType=sort_default_desc&arriveCountry=2456&showType=grid&pageSize=48&page="+str(2*k+1)
    res=requests.get(url)
    res.encoding='utf-8'
    root=etree.HTML(res.text)
    sku=root.xpath('//li[@class="goods-item"]/@data-spu')
    try:
        for j in range(len(sku)):
            try:
                print("2---正在获取{}商品!".format(sku[j]))
                time.sleep(random.randint(5,8))
                url1="https://www.joybuy.com/product/" + str(sku[j])+'.html'
                res = requests.get(url1)
                root = etree.HTML(res.text)
                ss=root.xpath('//script/text()')[0].split()
                vvid=re.match( r'(.*?)commentVersion:\'(.*?)\'(.*?)',''.join(ss)).group(2)
                # https://ipromo.joybuy.com/api/promoinfo/getInfo.html?json={%22channel%22:%222%22,%22curList%22:%22USD%22,%22site%22:1,%22skuId%22:1248010}
                            ...
                            for com in coms:
                                ...
                                try:
                                    with connection.cursor() as cursor:
                                        sql = " insert into `jd_180831_zhongbiao`(`proid`,`prourl`,`proprice`,`goodRateShow`,`commentCount`," \
